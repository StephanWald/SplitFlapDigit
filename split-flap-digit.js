class SplitFlapDigit extends HTMLElement {
  ...
  static get observedAttributes() {
    return ['value', 'alphabet', 'speed', 'mute', 'volume'];
  }
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    if (name === 'mute' && oldValue !== newValue) {
      this.updateAudioVolume();
    }
    if (name === 'volume' && oldValue !== newValue) {
      this.updateAudioVolume();
    }
  }
  ...
  setupAudio() {
    try {
      ...
      // Set initial volume based on mute and volume attributes
      if (this.getMute()) {
        this.audio.volume = 0;
      } else {
        this.audio.volume = this.getVolume();
